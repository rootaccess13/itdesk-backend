...
const allowCors = fn => async (req, res) => {
  res.setHeader('Access-Control-Allow-Credentials', true);  // Allow cookies in CORS requests
  res.setHeader('Access-Control-Allow-Origin', 'https://itdesk-frontend.vercel.app'); // Allow your frontend domain
  res.setHeader('Access-Control-Allow-Methods', 'GET, OPTIONS, PATCH, DELETE, POST, PUT'); // Allowed HTTP methods
  ...
  // If the method is OPTIONS, immediately respond with 200 status
  if (req.method === 'OPTIONS') {
    res.status(200).end();
    return;
  }
  
  // Pass to the actual handler (controller function)
