...
router.get('/', async (req, res) => {
  try {
    ...
    if (escalationLevel === 'administrator' || escalationLevel === 'staff') {
      // For 'administrator' or 'staff', use status only as filter
      if (status) {
        filter.status = status;
      }
    } else {
      // For other escalation levels, use both status and escalationLevel as filter
      if (status) {
        filter.status = status;
      }
      if (escalationLevel) {
    ...
    const tickets = await Ticket.find(filter).populate('assignedTo').skip(skip).limit(limit);
...
router.post('/create', upload.array('attachments', 10), async (req, res) => {
  ...
  const attachments = req.files ? req.files.map(async (file) => {
    const uploadParams = {
  ...
  try {
    ...
    const assets = await Promise.all(resolvedAttachments.map(async (attachment, index) => {
      const newAsset = new Asset({
        category: 'Ticket',
        assetTag: `${ticketNumber}-${index + 1}`,
        assetName: attachment.filename,
        assetType: 'Tickets',
        assetPath: attachment.url,  // S3 URL
        folder: folder._id,
      });
      await newAsset.save();
    ...
    // Save the updated folder with the new assets
    await folder.save();
    ...
    // Add timeline event
    await addTimelineEvent(newTicket._id, newTicket.assignedTo, newTicket.status, 'The ticket was created successfully.');
...
// @route   GET api/tickets/count/personal
// @desc    Get ticket count by status
// @access  Public
router.get('/count/personal/:id', async (req, res) => {
  try {
    ...
    const filter = { assignedTo: id }; // Include user ID in the filter
...
router.put('/edit/:id', upload.array('attachments', 10), checkRole(['administrator', 'staff', 'hardware-team', 'software-team']), async (req, res) => {
  const { id } = req.params;
  const currentUser = req.user;
  const { ticketNumber, title, description, status, priority, type, assignedTo, dueDate, comment, escalationLevel } = req.body;
  ...
  try {
    ...
    const updatedTicket = {
      ...
      type,
      severityLevel: 'Minor',
      escalationLevel, // Update this field from req.body
      dueDate,
      assignedTo: assignedToId,
      // attachments handling code (if needed)
    };

    const ticket = await Ticket.findByIdAndUpdate(id, { $set: updatedTicket }, { new: true });

    // Add timeline event
    await addTimelineEvent(id, currentUser._id, status, comment || 'The ticket was updated with new information.');
...
router.get('/export', checkRole(['administrator', 'staff']), async (req, res) => {
  try {
    ...
    const csv = json2csv(tickets.map(ticket => ({
      ...
      type: ticket.type,
      assignedTo: ticket.assignedTo ? `${ticket.assignedTo.username} (${ticket.assignedTo.email})` : 'Unassigned',
...
router.patch('/:id/update/status', async (req, res) => {
  ...
  try {
    ...
    // Add timeline event
    await addTimelineEvent(id, currentUser._id, status || 'The ticket status was updated.');
